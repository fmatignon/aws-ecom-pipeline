{#
"""
Staging model for orders that cleans, types, deduplicates, and adds QA flags
to the bronze orders table. Normalizes column names and values for downstream use.
"""
#}
...
with source as (
    select *
    from {{ source('ecom_bronze', 'orders') }}
    {% if is_incremental() %}
