{#
"""
Staging model for shipments that cleans, types, deduplicates, and normalizes
the bronze shipments table. Maps hyphenated column names to snake_case and
parses varchar date fields to timestamps with error flags.
"""
#}
...
with source as (
    select *
    from {{ source('ecom_bronze', 'shipments') }}
    {% if is_incremental() %}
