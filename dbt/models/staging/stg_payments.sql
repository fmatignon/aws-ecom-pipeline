{#
"""
Staging model for payments that cleans, types, deduplicates, and normalizes
the bronze payments table. Maps hyphenated column names to snake_case and
...
with source as (
    select *
    from {{ source('ecom_bronze', 'payments') }}
    {% if is_incremental() %}
